    ...
    def forward(self, x, return_attn=False):
        ...
        if self.flash and not return_attn:
            ...
            if hasattr(self, "bias"):
                ...
                causal = torch.tril(torch.ones(T, T, dtype=torch.bool, device=x.device)).view(1, 1, T, T)
...
class AOGPT(nn.Module):
    ...
    def forward(self, idx, mode='Random', orders=None, random_ratio=None, return_attentions=False):
        if mode is None:
        ...
        if mode is None:
            return self.forward_fn(idx, orders, return_attentions=return_attentions)
        elif mode == 'AR':
