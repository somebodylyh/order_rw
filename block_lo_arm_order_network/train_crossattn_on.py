"""Train CrossAttentionOrderNetwork on DP next-step labels.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train CrossAttentionOrderNetwork")
...
def main():
    ...
    output_name = f"crossattn_on_N{args.num_blocks}_best.pt"

    n_params = sum(p.numel() for p in model.parameters())
    print(f"Model: crossattn, params: {n_params:,}")
    ...
    try:
        ...
        dp_ratio = float(ratio.mean())
    except (FileNotFoundError, KeyError, ValueError) as e:
        print(f"Skipping DP eval: {e}")
        dp_ratio = 0.0
