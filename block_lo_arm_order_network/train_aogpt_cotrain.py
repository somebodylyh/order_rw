"""
Train AO-GPT with ON-sampled per-batch orders (Plan A: α annealing + τ floor).

Each training step:
  1. Sample a batch of sequences (by index from train pool)
  2. Compute α (sigmoid 0→1) and τ (sigmoid init→final, floor at final)
  3. For each sequence, with prob α use ON-sampled order (temp=τ), else random permutation
  4. Convert physical N16 order → model-coord token order
  5. AOGPT.forward_fn(idx, token_orders) → NLL → backprop AOGPT only

ON is frozen. α ramps 0→1 (sigmoid), τ decays init→final (sigmoid, never below final).
Per-step entropy, unique ratio, and α value logged to detect order collapse early.

Usage:
    python -u train_aogpt_cotrain.py --device cuda:0 --max-iters 10000 --batch-size 8
"""
...
@torch.no_grad()
def evaluate_aogpt(model, idx_all, baseline_token_order_eval, l2r_token_order_eval,
                   eval_indices, args):
    """Compute AR loss, Random loss, ON-greedy loss, and L2R loss on validation set."""
    model.eval()
    ...
    for seq_idx in eval_list:
        ...
        _, on_loss = model.forward_fn(idx, baseline_token_order_eval)
        order_losses.append(on_loss.item())

        _, l2r_loss = model.forward_fn(idx, l2r_token_order_eval)
    ...
    return {
        ...
        "l2r_loss": float(np.mean(l2r_losses)),
    }
...
def get_alpha(iter_num, args):
    """Sigmoid schedule 0 → 1, centered at max_iters/2.

    α controls the proportion of ON-sampled orders vs random permutations.
    α=0 → all random, α=1 → all ON. Sigmoid shape gives slow start, fast mid, slow end.
    """
    if iter_num >= args.max_iters:
        return 1.0
    x = (iter_num / max(args.max_iters, 1) - 0.5) * args.alpha_steepness
    return 1.0 / (1.0 + math.exp(-x))


def get_tau(iter_num, args):
    """Sigmoid decay tau from tau_init → tau_final, floor at tau_final.

    τ controls ON sampling sharpness. τ_final is a hard floor (never below 0.5)
    to guarantee per-batch order diversity even at α=1.
    """
    if iter_num >= args.max_iters:
        return args.tau_final
    x = (iter_num / max(args.max_iters, 1) - 0.5) * args.tau_steepness
    return args.tau_final + (args.tau_init - args.tau_final) * (1.0 - 1.0 / (1.0 + math.exp(-x)))
...
def parse_args():
    p = argparse.ArgumentParser(description="Train AO-GPT with ON-sampled per-batch orders (co-train Round 0)")
    ...
    p.add_argument("--tau-init", type=float, default=1.5)
    p.add_argument("--tau-final", type=float, default=0.7)
    p.add_argument("--tau-steepness", type=float, default=10.0,
                   help="Sigmoid steepness for tau decay (higher = sharper transition)")
    p.add_argument("--alpha-steepness", type=float, default=10.0,
                   help="Sigmoid steepness for alpha ramp (higher = sharper transition)")
...
def main():
    ...
    print(f"Schedule: α sigmoid 0→1 (steepness={args.alpha_steepness}), "
          f"τ sigmoid {args.tau_init}→{args.tau_final} (steepness={args.tau_steepness})", flush=True)

    # ── Load AO-GPT (unfrozen) ──────────────────────────────────────────────
    print("Loading AO-GPT (unfrozen)...", flush=True)
    if args.smoke:
        cfg = AOGPTConfig(block_size=SEQ_LEN, vocab_size=50257, n_layer=2, n_head=2, n_embd=64)
    ...
    # Baseline greedy order on mean train A (for eval comparison only)
    ...
    # L2R token order (physical 0..15 → model coords) for eval comparison
    l2r_n16_order = torch.arange(N16, device=device).unsqueeze(0)
    l2r_token_order = phys_n16_block_order_to_model_token_order(
        l2r_n16_order, block_perm
    )
    print(f"L2R token order computed for eval reference", flush=True)
    ...
    for iter_num in range(args.max_iters):
        ...
        # 2. Compute current alpha and tau
        alpha = get_alpha(iter_num, args)
        tau = get_tau(iter_num, args)

        # 3. Sample ON orders + random orders, mix per alpha (no grad — ON frozen)
        with torch.no_grad():
            n16_on, _, entropies = sample_one_order_per_seq(
                on_model, A_batch, temperature=tau
            )
            n16_rand = torch.stack([
                torch.randperm(N16, device=device) for _ in range(args.batch_size)
            ])
            use_on = (torch.rand(args.batch_size, device=device) < alpha)
            n16_orders = torch.where(
                use_on.unsqueeze(-1), n16_on, n16_rand
            )
        ...
        on_frac = use_on.float().mean().item()
        ...
        if iter_num % args.log_interval == 0 or iter_num == 0:
            ...
            print(f"Iter {iter_num:5d}/{args.max_iters} | "
                  ...
                  f"lr={lr:.2e} | α={alpha:.3f} (on={on_frac:.2f}) | τ={tau:.3f} | "
        ...
        if iter_num % args.eval_interval == 0 or iter_num == args.max_iters - 1:
            metrics = evaluate_aogpt(
                aogpt, idx_all_dev, baseline_token_order, l2r_token_order,
                val_indices, args
            )
            print(f"  [Eval @{iter_num:5d}] AR_loss={metrics['ar_loss']:.4f} | "
                  f"L2R_loss={metrics['l2r_loss']:.4f} | "
                  f"Random_loss={metrics['random_loss']:.4f} | "
                  f"ON_loss={metrics['on_loss']:.4f}", flush=True)
    ...
    # Load original ych ckpt to preserve model_args + data_permutation,
    ...
    state = best_state if best_state else {k: v.detach().cpu().clone() for k, v in aogpt.state_dict().items()}
    for k in list(state.keys()):
        clean = k.replace("_orig_mod.", "")
        if clean != k:
            state[clean] = state.pop(k)
    src["model"] = state
