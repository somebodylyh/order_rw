...
(per_head_order_scan none_mode='b0', one head) — preventing the B_train≠B_hook
...
def extract_selected_head_A_for_batch(model, idx_batch, head, clean_perm, device, probe_orders):
    """Forward `idx_batch` under `probe_orders`, extract one head's B0 block graph.

    Mirrors per_head_order_scan's offline extraction (none_mode='b0') for the
    single head (l, h): returns (batch, N, N) float32 physical-frame block graphs
    (diagonal zeroed). `probe_orders` are model-coordinate token orders (batch, SEQ_LEN).
    """
    l, h = head
    ...
    for bi in range(Bsz):
        A[bi] = _attn_to_A_block_b0_vec(attn_batch[l, bi, h], probe_np[bi], inv_perm)
...
class HookOrderProvider:
    ...
    def __init__(self, g_beta_ckpt, head, clean_perm, refresh_every=1,
                 mode="argsort", tau=1.0, seed=0, device="cuda:0"):
