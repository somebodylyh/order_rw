#!/bin/bash
# Overnight chain (continuous data, no overfit):
#   1) random-order baseline 0->50k  (saves ckpt_step5000.pt)
#   2) from that 5k ckpt: frozen g_beta hook, alpha genuinely ramps 0->1 over 5k steps
set -u
...
echo "[$(date)] === ARM A: random baseline (continuous) ==="
rm -rf "$RAND"
...
echo "[$(date)] === ARM B: g_beta hook from 5k ckpt (alpha 0->1 over 5k, ramp-from-resume) ==="
rm -rf "$GBETA"
