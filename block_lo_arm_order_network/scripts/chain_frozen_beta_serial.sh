#!/bin/bash
# Serial chain on GPU 0: g_β seed42 → frozen_beta seed2 → frozen_beta seed42 multi-start
# Launched after g_β seed42 completes. Run this ONCE g_β is done.
set -euo pipefail
...
# ═══════════════════════════════════════════════════════════════════
# Step 1: frozen_beta seed2 L0H2 from 10k → 60k (2nd seed for main table)
# ═══════════════════════════════════════════════════════════════════
...
echo ""
echo "=== [1/3] frozen_beta seed2 L0H2: 10k → 60k ==="
mkdir -p "$OUT_S2"
...
echo "=== [1/3] seed2 DONE at $(date) ==="

# ═══════════════════════════════════════════════════════════════════
# Step 2: frozen_beta seed42 L0H2 from 20k → 60k (multi-start, step savings)
# ═══════════════════════════════════════════════════════════════════
GBETA_42="$CD/batch_readout/logs/phase33_gbeta_seed42_from10k_l0h2/random_baseline_continuous_jun05/full/g_beta_best.pt"
RESUME_42_20K="$CD/probe_results/random_baseline_continuous_jun05/ckpt_step20000.pt"
OUT_42_20="$CD/probe_results/frozen_beta_seed42_from20k_l0h2"

echo ""
echo "=== [2/3] frozen_beta seed42 L0H2: 20k → 60k ==="
mkdir -p "$OUT_42_20"
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python -u "$CD/train_clean_aogpt.py" \
    --run-kind frozen_beta \
    --data-source continuous \
    ...
    --output-dir "$OUT_42_20" \
    --device "$GPU" \
    --lr 0.001 --min-lr 0.0001 --lr-decay-steps 50000 \
    --eval-interval 500 --log-interval 50 \
    --stream-eval-windows 2000 \
    2>&1 | tee "$OUT_42_20/train_log_${TS}.txt"
echo "=== [2/3] seed42 from20k DONE at $(date) ==="

# ═══════════════════════════════════════════════════════════════════
# Step 3: frozen_beta seed42 L0H2 from 40k → 60k (multi-start)
# ═══════════════════════════════════════════════════════════════════
RESUME_42_40K="$CD/probe_results/random_baseline_continuous_jun05/ckpt_step40000.pt"
OUT_42_40="$CD/probe_results/frozen_beta_seed42_from40k_l0h2"

echo ""
echo "=== [3/3] frozen_beta seed42 L0H2: 40k → 60k ==="
mkdir -p "$OUT_42_40"
PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True python -u "$CD/train_clean_aogpt.py" \
    --run-kind frozen_beta \
    --data-source continuous \
    ...
    --output-dir "$OUT_42_40" \
    --device "$GPU" \
    --lr 0.001 --min-lr 0.0001 --lr-decay-steps 50000 \
    --eval-interval 500 --log-interval 50 \
    --stream-eval-windows 2000 \
    2>&1 | tee "$OUT_42_40/train_log_${TS}.txt"
echo "=== [3/3] seed42 from40k DONE at $(date) ==="
