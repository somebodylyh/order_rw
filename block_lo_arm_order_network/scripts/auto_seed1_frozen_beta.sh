#!/bin/bash
# Auto-launch: wait for seed1 g_beta pretrain, then start frozen_beta hook to 60k.
# Usage: bash scripts/auto_seed1_frozen_beta.sh

set -euo pipefail

CD=$(dirname "$0")/..
GBETA_PT="$CD/batch_readout/logs/phase33_gbeta_seed1_from10k/random_baseline_continuous_jun09_seed1/full/g_beta_best.pt"
PHASE33_LOG="$CD/probe_results/phase33_gbeta_seed1_10k.log"
OUT_DIR="$CD/probe_results/frozen_beta_seed1_from10k"
TRAIN_LOG="$OUT_DIR/train_log.txt"

echo "[$(date)] Waiting for g_beta checkpoint: $GBETA_PT"
while [ ! -f "$GBETA_PT" ]; do
    sleep 30
    # also check that Phase 33 is still running
...
echo "[$(date)] g_beta ready. Launching frozen_beta seed1 → 60k on GPU 0"
...
PID=$!
echo "[$(date)] frozen_beta seed1 PID=$PID launched. Log: $TRAIN_LOG"
echo "PID=$PID" > "$OUT_DIR/launch_pid.txt"
