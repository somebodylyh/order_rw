"""Train Step-Wise MLP Reranker with attention-derived structural labels.

Flow:
1. Load A matrices, old ON (frozen)
2. Generate per-step structural soft labels from A matrix
3. Train MLP reranker to predict structural labels
4. Validate with structural path weight metrics

NO AOGPT involvement. NLL is NOT used for training, tuning, or evaluation.
"""
...
# ── Label generation (structural, no AOGPT) ────────────────────────────────────

def generate_structural_labels(
    old_on_prior, A_batch, cfg
):
    """Generate per-step structural soft labels from A matrix.

    For each sequence, runs old ON autoregressively to get prefix,
    then computes structural label for each candidate.

    Returns:
        labels: list of dicts with seq_idx, step, features (K,F), struct_targets (K,)
    """
    ...
    for s in tqdm(range(num_seqs), desc="Generating structural labels"):
        A_s = A_batch[s:s + 1]  # (1, N, N)
        A_np = A_s[0].cpu().numpy()

        # Get old ON full order for prefix distribution
        ...
        for t in range(N):
            ...
            # Structural label from A matrix
            prefix_np = prefix.cpu().numpy()
            cand_np = candidate_set.cpu().numpy()
            struct_target = build_attention_labels(
                A_np, prefix_np, cand_np,
                mode=cfg.label_mode,
                tau=cfg.label_tau,
            )  # (K,) soft label
            ...
            all_labels.append({
                ...
                "struct_targets": torch.from_numpy(struct_target).float(),
            })
...
def train_mlp_reranker_struct(labels, cfg, device):
    """Train MLP to predict structural attention labels."""
    ...
    print(f"  Model: {'PooledMLP' if cfg.use_pooled_context else 'MLP'}, "
          ...
          f"loss={cfg.loss_type}, label_mode={cfg.label_mode}, tau={cfg.label_tau}")
...
def sanity_overfit_struct(labels, cfg, device, n_seqs=10):
    """Train MLP on tiny subset to verify it can fit structural labels."""
    ...
    print(f"Sanity overfit: {len(subset_labels)} queries from {n_seqs} seqs, "
          f"feature_dim={feature_dim}, loss={cfg.loss_type}, pooled={cfg.use_pooled_context}")
...
@torch.no_grad()
def validate_reranker_struct(
    reranker, old_on_prior, A_batch, cfg
):
    """Validate reranker with structural path weight metrics. No NLL."""
    device = A_batch.device
    num_seqs = len(A_batch)
    ...
    for s in tqdm(range(num_seqs), desc="Validating (struct)"):
        A_s = A_batch[s:s + 1]
        ...
        # A-greedy order
        greedy_order = generate_order_A_greedy(A_s, mode=cfg.label_mode)[0]

        # Compute path weights
        A = A_np[s]
        mlp_weights.append(compute_path_weight(A, mlp_order.cpu().numpy(), mode=cfg.label_mode))
        old_on_weights.append(compute_path_weight(A, sigma_old.cpu().numpy(), mode=cfg.label_mode))
        greedy_weights.append(compute_path_weight(A, greedy_order.cpu().numpy(), mode=cfg.label_mode))

        # Random MC
        rand_ws = []
        for _ in range(cfg.num_random_mc):
            rand_order = np.random.permutation(cfg.num_blocks)
            rand_ws.append(compute_path_weight(A, rand_order, mode=cfg.label_mode))
...
def main():
    ...
    if not args.labels_file:
        args.labels_file = os.path.join(
            cfg.output_dir, f"labels_struct_{cfg.reranker_feature_set}_{cfg.label_mode}.pt"
        )
    if args.skip_labels and os.path.exists(args.labels_file):
        ...
        t0 = time.time()
        labels_train = generate_structural_labels(
            old_on_prior, A_train, cfg
        )
    ...
    if not args.skip_sanity:
        print("\n=== Sanity Overfit (Structural) ===")
        sanity_overfit_struct(labels_train, cfg, device, n_seqs=10)

    # ── Train ──
    print("\n=== Training MLP Reranker (Structural Labels) ===")
    t0 = time.time()
    mlp_reranker = train_mlp_reranker_struct(labels_train, cfg, device)
    print(f"Training took {time.time() - t0:.1f}s")
    print(f"MLP params: {mlp_reranker.count_parameters():,}")

    suffix = f"_struct_{cfg.reranker_feature_set}_{cfg.loss_type}"
    if cfg.use_pooled_context:
        suffix += "_pooled"
    ...
    print("\n=== Validation (Structural Metrics) ===")
    mlp_reranker.eval()
    results = validate_reranker_struct(
        mlp_reranker, old_on_prior, A_tune, cfg
    )

    print(f"\nVal results ({n_tune} seqs, mode={cfg.label_mode}):")
    print(f"  MLP reranker weight:    {results['mlp_weight']:.4f}")
