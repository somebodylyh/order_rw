...
def parse_args():
    ...
    parser.add_argument("--layer_reduce", type=str, default="mean", choices=["mean", "last", "single"])
    parser.add_argument("--head_reduce", type=str, default="mean", choices=["mean", "first", "single"])
    parser.add_argument("--specific_layer", type=int, default=0,
                       help="Layer index when --layer_reduce=single (default: 0)")
    parser.add_argument("--specific_head", type=int, default=0,
                       help="Head index when --head_reduce=single (default: 0)")
...
def resolve_data_permutation(checkpoint, num_blocks: int, block_len: int, model=None):
    ...
    permute_mode = data_permutation.get("permute_mode") or "block"
    ...
    # ── Normalize to nanogpt convention (block_perm = model→phys) ──
    # nanogpt-learned-order:  block_perm[m]=p  (model→phys)
    # block_lo_arm_order_network: block_perm[p]=m (phys→model), marked by
    #   convention="clean_phys_to_model". Swap so downstream code always sees
    #   block_perm=model→phys, inverse_block_perm=phys→model.
    convention = data_permutation.get("convention", "")
    if convention == "clean_phys_to_model":
        block_perm, inverse_block_perm = inverse_block_perm, block_perm
    ...
    for export_type in EXPORT_TYPES:
        ...
        for frame in export_frames:
            ...
            metadata = {
                ...
                "specific_layer": args.specific_layer if args.layer_reduce == "single" else None,
                "specific_head": args.specific_head if args.head_reduce == "single" else None,
...
def main():
    ...
    for batch_idx in range(int(args.num_batches)):
        ...
        attn_batch = reduce_attention_batch(
            ...
            specific_head=args.specific_head,
        )
