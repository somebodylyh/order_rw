# WikiText103 seq80 permuted block1 try_2:
# Token-level seq80 port of the latest no-prior Attn-MLP method.
#
# Base training shape is inherited from seq80/permute/block1/random.py:
# - block_size=80, block_order_block_len=1, order_impl='token'
# - batch_size=256, gradient_accumulation_steps=2
# - permute_data=True, permute_seed=42, permute_mode='block'
# - lr/max_iters/eval cadence follow the seq80 random base.
#
# Schedule:
# - 0..18k: pure Random warmup. The Attn-MLP policy is lazy-initialized, so
#   MLP parameters and optimizer are not created before the selector fires.
# - iter 18k: select one head from current-frame without_none attention:
...
# - 18k..40k: train the MLP/order with directed-ribbon loss.
# - 40k..50k: freeze cached MLP order and continue backbone training.
#
# No-prior rule:
...
out_dir = 'out/base/permute/seq80/block1/out-wikitext103-seq80-try2-autohead-eigloss-withoutnone-directed-ribbon-lazyinit-warmup18k-anneal40k-fixed50k-h2048-b1-permute'
...
# Keep seq80 run stability conventions used by the seq80 online configs.
compile = False
eval_batch_size = 64

wandb_log = True
wandb_project = 'AOGPT-order-block-seq80-final'
wandb_run_name = 'seq80-try2-autohead-eigloss-withoutnone-directed-ribbon-lazyinit-warmup18k-anneal40k-fixed50k-h2048-b1-permute'
...
attn_mlp_policy_lazy_init_iter = 18000

# Placeholder until head_asym_selector assigns the selected head at iter 18000.
...
attn_mlp_policy_hidden_dims = '2048,2048'
...
attn_mlp_policy_log_input_attn_prefix = 'seq80_try2_autohead_eigloss_withoutnone_input_attn'
attn_mlp_policy_log_input_attn_out_dir = 'Report/MLP_loss_training/seq80_try_2/input_attn'
...
attn_mlp_policy_lr_anneal_start_iter = 18000
...
# and cached-order refresh continue until update_stop_iter=40000.
...
head_asym_selector_target_iter = 18000
head_asym_selector_split = 'train'
head_asym_selector_batch_size = 256
head_asym_selector_batches = 4
head_asym_selector_export_type = 'without_none'
head_asym_selector_out_dir = 'Report/MLP_loss_training/seq80_try_2/head_asym_selector'
...
head_asym_selector_loss_batch_size = 128
head_asym_selector_loss_candidate_batch_size = 2
