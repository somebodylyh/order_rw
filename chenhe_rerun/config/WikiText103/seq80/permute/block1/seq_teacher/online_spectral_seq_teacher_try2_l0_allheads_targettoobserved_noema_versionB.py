# WikiText103 seq80 permuted block1 seq-teacher try_2.
#
# Teacher-only follow-up to try_1. The training schedule, random seed, model
# size, sample counts, and no-EMA Version-B policy are kept matched. The only
# intended method change is the teacher readout:
# - use target_to_observed attention export
# - use layer 0 mean over all heads as the input matrix
#
...
out_dir = (
    ...
    'out-wikitext103-seq80-seqteacher-try2-online-spectral-'
...
wandb_run_name = (
    'seq80-seqteacher-try2-online-spectral-l0-allheads-targettoobserved-'
...
_report_root = (
    'Report/language/wikitext103/order_teacher_distribution/token/seq80/try_2'
)
head_signal_probe_out_dir = _report_root + '/online_training_probe'

# Latest seq readout: mean all heads in L0, then recover teacher order from
# target-to-observed attention with exposure correction in train.py.
...
head_signal_probe_export_type = 'target_to_observed'
...
online_spectral_policy_log_input_attn_prefix = (
    'seq80_seqteacher_try2_l0_allheads_targettoobserved_noema_versionB_input_attn'
)
