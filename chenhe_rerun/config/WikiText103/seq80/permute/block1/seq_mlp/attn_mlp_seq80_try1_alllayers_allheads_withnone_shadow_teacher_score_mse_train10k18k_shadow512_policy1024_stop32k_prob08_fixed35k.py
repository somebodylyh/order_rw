# WikiText103 seq80 permuted block1 Attn-MLP try_1.
#
# Token-level counterpart to the block64 try60 schedule:
# - inherit the seq80 random base, including 4 layers, 8 heads, 384 emb
# - use all-layer all-head mean attention as MLP input
# - use with_none attention export to match the seq teacher try_1 input family
...
out_dir = (
    ...
    'out-wikitext103-seq80-attnmlp-try1-alllayers-allheads-withnone-'
...
wandb_run_name = (
    'seq80-attnmlp-try1-alllayers-allheads-withnone-shadow-teacher-score-mse-'
...
# Mean over all layers and all heads. The with_none slice follows the token
# teacher try_1 input family while still producing an 80-token policy order.
...
_report_root = 'Report/language/wikitext103/mlp/token/seq80/try_1'
...
attn_mlp_policy_log_input_attn_prefix = (
    'seq80_attnmlp_try1_alllayers_allheads_withnone_shadow_teacher_score_mse_'
...
# Keep this first MLP try clean: MSE-to-teacher-score only.
...
assert bool(attn_mlp_policy_use_global)
assert str(attn_mlp_policy_export_type) == 'with_none'
