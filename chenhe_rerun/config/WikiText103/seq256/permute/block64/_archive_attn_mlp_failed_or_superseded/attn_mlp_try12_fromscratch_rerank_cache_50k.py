# WikiText103 seq256 permuted block64 try_12:
# from-scratch backbone + from-scratch trainable Attn-MLP.
#
# try_12 keeps try_11's 50k schedule and no-prior MLP objective, but changes
# how the cached order is chosen. Instead of always caching the MLP MAP order,
# it periodically reranks MLP/random/local proposal orders on train-split
# current-model full/prefix loss. OriginalL2R, original tau, oracle
# permutations, and hand-written original-order labels are not used for
# training, reranking, early stopping, or model selection. Tau remains a
# post-hoc diagnostic only.

out_dir = 'out/base/permute/seq256/block64/out-wikitext103-seq256-try12-fromscratch-rerank-cache-50k-b64-permute-block'
eval_interval = 1000
eval_iters = 200
log_interval = 10
...
wandb_run_name = 'seq256-try12-fromscratch-rerank-cache-50k-b64-permute'
...
# Same macro-schedule as try_11:
# 0-5k random with shadow MLP training, 5k-15k anneal, 15k-25k 100%
# MLP order while still trainable, then 25k-50k fixed cached order.
...
# No-prior robust cached-order selection. The score is computed only on
# train-split current-model full/prefix loss; no validation, original order,
# tau, or oracle order enters this path.
attn_mlp_policy_cache_rerank_enabled = True
attn_mlp_policy_cache_rerank_start_iter = 10000
attn_mlp_policy_cache_rerank_every = 1000
attn_mlp_policy_cache_rerank_train_batches = 2
attn_mlp_policy_cache_rerank_batch_size = 32
attn_mlp_policy_cache_rerank_candidates = 12
attn_mlp_policy_cache_rerank_sampled = 6
attn_mlp_policy_cache_rerank_random = 3
attn_mlp_policy_cache_rerank_local_swaps = 4
attn_mlp_policy_cache_rerank_prefix_k = 8
attn_mlp_policy_cache_rerank_prefix_weight = 0.05
