# WikiText103 seq256 permuted block64 try_29:
# From-scratch frozen-MLP integration using the latest try_28 two-hidden-layer
# distilled operator. No attention EMA is used and the MLP order is refreshed
# every training step until the fixed-order phase.
#
# Schedule:
# - 0..10k: Random warmup inside this run.
# - 10k..35k: every step, feed the current L0 layer-mean attention to the MLP;
#   train anneals Random -> MLP.
...
out_dir = 'out/base/permute/seq256/block64/out-wikitext103-seq256-try29-seed2027-fromscratch-frozen-try28-l0-layermean-fiedler-mlp-noema-update1-warmup10k-anneal35k-freeze35k-b64-permute-block'
...
wandb_run_name = 'seq256-try29-seed2027-fromscratch-frozen-try28-l0-layermean-fiedler-mlp-noema-update1-warmup10k-anneal35k-freeze35k-b64-permute'
...
# L0 all-head mean, without_none, same input family as the try_28 student.
...
# No EMA: decay=0 means the cached matrix is exactly the latest collected
# attention matrix, so the MLP output order is used directly.
...
attn_mlp_policy_log_input_attn_prefix = 'try29_seed2027_fromscratch_frozen_try28_l0_layermean_fiedler_mlp_noema_update1_input_attn'
attn_mlp_policy_log_input_attn_out_dir = 'Report/language/wikitext103/mlp/distillation/try_29/input_attn'
