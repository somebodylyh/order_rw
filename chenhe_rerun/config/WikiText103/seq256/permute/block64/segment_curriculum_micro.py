# Conservative unit-level micro-segment curriculum config for seq256/block64.
# Usage:
#   python scripts/runner/hierarchical_segment_curriculum_runner.py config/WikiText103/seq256/permute/block64/segment_curriculum_micro.py

config = 'config/WikiText103/seq256/permute/block64/random.py'

train_out_dir = 'out/curriculum/permute/seq256/block64/out-wikitext103-seq256-random-b64-permute-hcurr-micro'
benchmark_root = 'Report/curriculum/permute/seq256/block64/hierarchical_block64_micro'

wandb_project = 'AOGPT-order-block-64'
wandb_run_name = 'seq256-random-b64-permute-hcurr-micro'

warmup_iters = 8000
stage_iters = 8000
num_curriculum_stages = 4
segment_guided_ratios = '1,1,1,1'
segment_max_lens = '999999,999999,999999,999999'
segment_max_units_per_order = 999999
segment_use_all_units = True
segment_top_k_pairs = 96

benchmark_batch_size = 1
pair_mining_batches = 24
pair_eval_batch_size = 8
pair_chunk_size = pair_eval_batch_size
forward_eval_batch_size = benchmark_batch_size * pair_chunk_size

aggregation_margin_threshold = 0.03
pair_aggregation_mode = 'token_micro_segments'
benchmark_num_levels = 1
aggregate_top_k_pairs = 128
pair_score_k = 2
pair_score_mode = 'signed_drop'
tv_weight = 0.3
drop_weight = 0.2

pair_mining_mode = 'attention_pruned'
attn_top_k = 6
attn_num_batches = 24
attn_batch_size = 32
attn_mode = 'Random'
attn_symmetrize = 'mean'
attn_export_type = 'with_none'

token_micro_topk = 4
token_micro_min_affinity = -1e30
token_micro_max_degree = 6
token_micro_max_segment_size = 4
token_micro_min_segment_size = 2
token_micro_min_density = 0.4
token_micro_num_order_candidates = 5
token_micro_eval_batches = 32
token_micro_eval_batch_size = 32
token_micro_random_suffixes = 4
token_micro_random_orders = 4
token_micro_min_pair_consistency = 0.01
token_micro_reverse_margin_threshold = 0.02
token_micro_random_margin_threshold = 0.01
token_micro_keep_singletons = False
token_micro_max_accepted_segments = 0
