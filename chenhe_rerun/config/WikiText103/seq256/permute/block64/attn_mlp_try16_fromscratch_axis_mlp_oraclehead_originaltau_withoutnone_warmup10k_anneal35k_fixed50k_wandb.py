...
# Train from scratch. The first 10k iterations are a fresh Random warmup from
# this run's seed; no clean Random-10k checkpoint is copied or resumed.
#
# At iter 10000, run the same oracle/prior head-selection ablation as the old
# try16: recover direct-asym-eig raw/reverse orders for every head and choose
# the head whose best orientation has the highest OriginalL2R tau. This prior
# is used only to select the head.
#
# After the head is assigned, the order is produced only by the frozen try15
# sign-invariant eig-axis Attn-MLP from the selected head's without_none
# attention matrix. The MLP is not updated in the main run.
...
out_dir = 'out/base/permute/seq256/block64/out-wikitext103-seq256-try16-fromscratch-seed2026-axis-mlp-oraclehead-originaltau-withoutnone-warmup10k-anneal35k-fixed50k-b64-permute-block'
...
wandb_run_name = 'seq256-try16-fromscratch-seed2026-axis-mlp-oraclehead-originaltau-withoutnone-warmup10k-anneal35k-fixed50k-b64-permute'
...
attn_mlp_policy_path = 'checkpoints/attn_mlp_distillation/try15_sign_invariant_axis_mlp/best_by_val_axis_tau.pt'
...
# 0..10k fresh Random warmup, 10k..35k Random/MLP anneal, then fixed cached MLP
# order to 50k.
...
attn_mlp_policy_log_input_attn_prefix = 'try16_fromscratch_axis_mlp_oraclehead_originaltau_input_attn'
attn_mlp_policy_log_input_attn_out_dir = 'Report/MLP_distillation/try_16/main_block64/input_attn'
...
head_asym_selector_out_dir = 'Report/MLP_distillation/try_16/main_block64/head_asym_selector'
...
head_asym_selector_seed = 975318642

# This is intentionally an oracle/prior ablation for this test only.
