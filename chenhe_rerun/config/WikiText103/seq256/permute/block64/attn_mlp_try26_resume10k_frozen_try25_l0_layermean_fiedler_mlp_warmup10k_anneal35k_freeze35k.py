...
# Resume from the clean Random 10k checkpoint and replace the graph-Laplacian
# L0 layer-mean Fiedler operator with the frozen try_25 distilled Attn-MLP.
#
# This is a backbone integration test only. The MLP is loaded from checkpoint
# and kept frozen; it predicts an order from the current L0 all-head mean
# without_none attention matrix.
...
out_dir = 'out/base/permute/seq256/block64/out-wikitext103-seq256-try26-resume10k-frozen-try25-l0-layermean-fiedler-mlp-warmup10k-anneal35k-freeze35k-b64-permute-block'
...
wandb_run_name = 'seq256-try26-resume10k-frozen-try25-l0-layermean-fiedler-mlp-warmup10k-anneal35k-freeze35k-b64-permute'
...
attn_mlp_policy_path = 'checkpoints/attn_mlp_distillation/try25_joint_try20_try24_l0_layermean_fiedler_mlp_h2048_2048_1024/best_by_val_tau.pt'
attn_mlp_policy_freeze = True
attn_mlp_policy_train_loss = 'none'

# L0 all-head mean, matching the latest pairwise-max Fiedler distribution line.
...
# try_25 distillation trained with higher logits meaning earlier blocks.
attn_mlp_policy_order_mode = 'argsort_desc'
attn_mlp_policy_per_sample = False
attn_mlp_policy_fallback = 'random'

# Match the distribution bridge schedule:
# - <=10k: the copied base checkpoint has already completed Random warmup.
# - 10k..35k: refresh MLP order every 20 steps; train anneals Random -> MLP.
# - 35k..50k: stop refreshing and train on the final cached MLP order.
...
# Smooth the cached order updates without training the MLP itself.
...
attn_mlp_policy_log_input_attn_prefix = 'try26_resume10k_frozen_try25_l0_layermean_fiedler_mlp_input_attn'
attn_mlp_policy_log_input_attn_out_dir = 'Report/language/wikitext103/mlp/distillation/try_26/input_attn'
