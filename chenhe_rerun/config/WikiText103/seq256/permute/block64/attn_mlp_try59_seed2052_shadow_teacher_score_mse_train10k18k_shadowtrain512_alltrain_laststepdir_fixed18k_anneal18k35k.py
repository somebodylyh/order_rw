# WikiText103 seq256 permuted block64 try_59:
# Same as try57, but with seed=2052 and policy attention averaged over 512 samples.
# Shadow-train a random-init score MLP from
# 10k to 18k, then use the latest distribution Version-B schedule: MLP
# probability 0.0 -> 0.8 while refreshing until 32k, then fixed cached MLP
# order 0.8 -> 1.0 by 35k, then deterministic fixed order.
...
out_dir = 'out/base/permute/seq256/block64/out-wikitext103-seq256-try59-seed2052-shadow-teacher-score-mse-train10k18k-shadowtrain512-alltrain-laststepdir-policyattn512-linearlabel-stop32k-prob08-freeze35k-b64-permute-block'
...
wandb_run_name = 'seq256-try59-seed2052-shadow-teacher-score-mse-train10k18k-shadowtrain512-alltrain-laststepdir-policyattn512-linearlabel-stop32k-prob08-freeze35k-b64-permute'
...
#            Each shadow train item averages 512 raw samples:
#            4 optimizer iters * 2 micro-batches * 64 samples = 512.
#            All shadow items are used as train data, with no val split.
#            Direction labels use the last optimizer step inside each item.
# 18k..32k: stop MLP optimizer updates, keep refreshing cached MLP orders, and
#            average 512 raw samples per policy input attention update.
...
attn_mlp_policy_attention_batches_per_update = 4
...
attn_mlp_policy_shadow_mse_train_items_per_update = 1
...
attn_mlp_policy_shadow_mse_log_path = 'Report/language/wikitext103/mlp/distillation/try_59/input_attn/attn_mlp_shadow_mse_history.jsonl'
...
attn_mlp_policy_log_input_attn_prefix = 'try59_seed2052_shadow_teacher_score_mse_train10k18k_shadowtrain512_alltrain_laststepdir_linearlabel_stop32k_prob08_freeze35k_input_attn'
attn_mlp_policy_log_input_attn_out_dir = 'Report/language/wikitext103/mlp/distillation/try_59/input_attn'
