# try_22: Frozen L0 layer-mean Fiedler-distilled Attn-MLP order policy.
#
# Paired comparison for the latest layer-level distribution method:
# - distribution baseline: L0 all-head mean, without_none, pairwise_max_fiedler,
#   update20, warmup10k, anneal to 35k, then fixed to 50k.
# - this run: feed the same L0 all-head mean attention matrix into the frozen
#   try_21 MLP and use its predicted order with the same schedule.
#
# The MLP is not trained in this run. This tests whether the distilled operator
# can replace the online Fiedler decomposition inside the backbone.
...
out_dir = 'out/base/permute/seq256/block64/out-wikitext103-seq256-try22-frozen-l0-layermean-fiedler-mlp-seed2028-warmup10k-anneal35k-freeze35k-b64-permute-block-50000-iters'
...
wandb_run_name = 'seq256-try22-frozen-l0-layermean-fiedler-mlp-seed2028-warmup10k-anneal35k-freeze35k-b64-permute-50000-iters'
...
# L0 all-head mean, matching distribution try_23/24/25 head_signal_probe_heads='0:mean'.
...
attn_mlp_policy_log_input_attn_prefix = 'try22_l0_layermean_fiedler_mlp_input_attn'
attn_mlp_policy_log_input_attn_out_dir = 'Report/language/wikitext103/mlp/distillation/try_22/input_attn'
