# Scheme B variant: start order policy at 8k and freeze the MAP order at 35k.
#
# Compared with online_spectral_try19_bridge_oriented_distribution_map_update20_warmup5k_anneal15k:
# - 0..8000: pure random training orders, no head-signal bridge/probe cost.
# - 8000..18000: linearly anneal policy usage from 0 to 1.
# - 18000..35000: train from the try19-oriented distribution MAP, still updating every 20 steps.
# - 35000..50000: freeze updates and train with the current MAP order.
#
# The method is still no-prior B: current attention proposes head orders, current
...
out_dir = 'out/base/nonpermute/seq256/block64/out-wikitext103-seq256-online-spectral-try19-bridge-oriented-distribution-map-update20-start8k-anneal18k-freeze35k-b64-nonpermute-50000-iters'

wandb_log = True
wandb_project = 'AOGPT-order-block-64-final'
wandb_run_name = 'seq256-online-spectral-try19-bridge-oriented-distribution-map-update20-start8k-anneal18k-freeze35k-b64-nonpermute-50000-iters'
...
head_signal_probe_out_dir = 'Report/head_singal_Stable/try19_bridge_oriented_distribution_map_update20_start8k_anneal18k_freeze35k/online_training_probe'
