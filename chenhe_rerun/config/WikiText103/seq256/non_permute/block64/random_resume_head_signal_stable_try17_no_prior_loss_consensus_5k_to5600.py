# Continue from the same 5k checkpoint with a fully no-prior loss-consensus rule.
# No original-direction, tau, PPL, target-position, position-embedding, or
# historical-sign signal is used for orientation.
#
# The rule:
# 1. recover each head's unsigned attention-spectral order axis;
# 2. orient S vs reverse(S) by current-model conditional loss;
# 3. aggregate same-probe signed precedence matrices across heads;
# 4. re-orient each head by leave-one-out agreement with that same-step
#    model-internal consensus.

out_dir = 'out/base/nonpermute/seq256/block64/out-wikitext103-seq256-random-b64-head-signal-stable-try17-no-prior-loss-consensus-5k-to5600'
eval_interval = 200
eval_iters = 100
log_interval = 20
...
wandb_run_name = 'seq256-random-b64-head-signal-stable-try17-no-prior-loss-consensus-5k-to5600'
...
head_signal_probe_out_dir = 'Report/head_singal_Stable/try_17/online_training_probe'
