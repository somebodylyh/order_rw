...
# head-order orientation inside the training loop. Compared with try 2, the
# probe uses a deterministic audit seed at every probe step and restores RNG
# state afterward, so the audit set is fixed while the model remains current.

out_dir = 'out/base/nonpermute/seq256/block64/out-wikitext103-seq256-random-b64-head-signal-stable-try3-deterministic-probe-5k-to5600'
eval_interval = 200
eval_iters = 100
log_interval = 20
...
wandb_run_name = 'seq256-random-b64-head-signal-stable-try3-deterministic-probe-5k-to5600'
...
head_signal_probe_out_dir = 'Report/head_singal_Stable/try_3/online_training_probe'
