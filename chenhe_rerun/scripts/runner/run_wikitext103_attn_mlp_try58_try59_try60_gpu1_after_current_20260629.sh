...
RUN_DIR="${RUN_DIR:-${ROOT}/Report/logs/wikitext103_attn_mlp_try58_try59_try60_gpu1_after_current_${STAMP}}"
...
TRY60_CONFIG="config/WikiText103/seq256/permute/block64/attn_mlp_try60_seed2053_shadow_teacher_score_mse_train10k18k_shadowtrain512_alltrain_laststepdir_policyattn1024_fixed18k_anneal18k35k.py"
...
terminate() {
  log "received termination; stopping try58/try59/try60 GPU1 queue"
  printf 'terminated\n' > "$STATUS_FILE"
...
preflight() {
  ...
  local missing=0
  for config in "$TRY58_CONFIG" "$TRY59_CONFIG" "$TRY60_CONFIG"; do
    if [[ ! -f "$config" ]]; then
...
queue=try58_then_try59_then_try60
...
try60_config=${TRY60_CONFIG}
sequence=try58,try59,try60
...
main() {
  ...
  log "try58/try59/try60 GPU1 serial queue started"
  log "run_dir=${RUN_DIR}"
  log "cancel with: kill -TERM -- -${QUEUE_PGID}"

  run_one try58 "$TRY58_CONFIG"
  run_one try59 "$TRY59_CONFIG"
  run_one try60 "$TRY60_CONFIG"

  printf 'completed\n' > "$STATUS_FILE"
  log "try58/try59/try60 GPU1 serial queue completed"
}
