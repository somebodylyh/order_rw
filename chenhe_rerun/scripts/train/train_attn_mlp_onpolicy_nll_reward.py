...
LOSS_NAME = "on-policy frozen-NLL REINFORCE reward"


def parse_args():
    parser = argparse.ArgumentParser(description="Train Attn-MLP with online sampled-order frozen-NLL reward.")
...
def train_order_evals(args):
    ...
    return train_states(args) * (
        int(args.sampled_orders_per_state) + int(args.random_baseline_orders) + int(loss_axis_evals)
    )
...
For each training state, the script samples one token batch, uses a random
reveal order only to extract current-frame attention and current-frame block
loss features, then evaluates MLP-sampled orders on the same token batch.
...
- optional current-block-loss axis sign selected by frozen NLL on the same
  token batch, when `lambda_loss_axis_pair > 0`
...
def make_failure_analysis(summary):
    ...
    if not success:
        if train_reward > 0.0 and not (nll_ok or dist_ok):
            ...
            next_step = "revise the policy-gradient reward estimator or use a learned value baseline tied to the same current-frame features"
...
def write_try_docs(args, summary=None):
    ...
    readme = f"""# Attn-MLP On-Policy NLL Reward Try
...
def main():
    ...
    summary = {
        "script": "scripts/train/train_attn_mlp_onpolicy_nll_reward.py",
    ...
    torch.save(
        {
            ...
            "policy_type": "attention_onpolicy_nll_reward_mlp",
