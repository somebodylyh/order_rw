...
- **Slot-only predictor:** `B_hat[i,j]=mean_{train texts}B[i,j]`; **train/held-out split over TEXT samples, not edges**; predictor sees no held-out-text activations. Report global total-entry held-out R² for both raw and normalized B, together with a size-matched randomized-null mean/std over 16 non-overlapping deterministic seed-block bases `0,M,...,15M` and null-adjusted excess `(r2-null_mean)/(1-null_mean)`. Normalized R² alone is not strong B evidence: row normalization gives the randomized synthetic null about 0.70.
